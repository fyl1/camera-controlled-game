document.addEventListener("DOMContentLoaded", function (event) {
  const game_start_element = document.getElementById("game_start");
  const game_start_container = document.getElementById("game_start_container");
  const game_container = document.getElementById("game_container");
  const loadingItem = document.getElementById("loading");

  game_start_element.addEventListener("click", function (evt) {
  ...
  function gameControllHand(results) {
    if (!load_done) {
      load_done = true;
    ...
    if (results.multiHandLandmarks) {
      for (const landmarks of results.multiHandLandmarks) {
        let hand_y = landmarks[5].y;

        let next_y = hand_y * ((RightmaxPaddleY - grid) * 2);

        rightPaddle.y = next_y;

        console.log("hand_y=" + hand_y + ", next_y = " + next_y);
  ...
  hands.onResults(gameControllHand);
